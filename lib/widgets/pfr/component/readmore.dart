...
class ReadMoreController {
  List<ReadMoreTextState> _state = [];

  void attach(ReadMoreTextState state) {
    if (!_state.contains(state)) {
      _state.add(state);
    }
  }

  void detach(ReadMoreTextState state) {
    if (_state.contains(state)) {
      _state.remove(state);
    }
  }
  ...
  collapse() {
    _state.forEach((st) => st.changeExpand(true));
  }

  expand() {
    _state.forEach((st) => st.changeExpand(false));
  }
}
